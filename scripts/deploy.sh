...
#
#   bash scripts/deploy.sh            # sync + npm ci + validate + restart Slack agent
...
echo ">> restarting Slack agent service"
ssh "$HOST" "systemctl restart warmup-agent.service && systemctl is-active warmup-agent.service"

echo ">> done. server is synced. crontab unchanged; Slack agent restarted."
